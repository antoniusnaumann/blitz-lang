...
fn run_internal(st: &Statement, vars: &mut HashMap<String, Value>, reg: &Registry) -> Value {
    match st {
        ...
        Statement::Expression(expression) => run_expr(expression, vars, reg),
    }
}

fn run_expr(expression: &Expression, vars: &mut HashMap<String, Value>, reg: &Registry) -> Value {
    match expression {
        Expression::Constructor(c) => {
            ...
            for init_arg in &c.args {
                let val = run_expr(init_arg.init.deref(), vars, reg);
                fields.insert(init_arg.label.name.clone(), val);
            }
        ...
        Expression::Call(call) => {
            ...
            let arg_vals: Vec<_> = call
                .args
                .iter()
                .map(|call_arg| run_expr(call_arg.init.as_ref(), vars, reg))
                .collect();
            ...
            let func = if has_any_labels && funcs.len() == 1 {
                ...
                reg.select_func(&call.name, funcs, &arg_vals, None, None)
            };
        ...
        Expression::Ident(ident) => vars.get(&ident.name).cloned().unwrap_or_else(|| {
            ...
            Value::Union(ident.name.clone(), Value::Void.into())
        }),
        Expression::Assignment(assignment) => {
            let rhs = run_expr(assignment.right.deref(), vars, reg);
        ...
        Expression::For(for_loop) => {
            let iter = run_expr(for_loop.iter.deref(), vars, reg);
            match iter {
                Value::List(values) => {
        ...
        Expression::While(while_) => {
            ...
            loop {
                let cond = run_expr(while_.cond.deref(), vars, reg);
                match cond {
                    Value::Bool(val) => {
                        if !val {
                            break;
        ...
        Expression::If(if_) => {
            let cond = run_expr(if_.cond.deref(), vars, reg);
            match cond {
                Value::Bool(val) => {
                    if val {
                        let mut result = Value::Void;
        ...
        Expression::BinaryOp(bin) if bin.op == Operator::Else => {
            let lhs = run_expr(bin.left.deref(), vars, reg);
            if lhs == Value::Union("none".into(), Value::Void.into()) {
                run_expr(bin.right.deref(), vars, reg)
            } else {
        ...
        Expression::Switch(switch) => {
            let cond_value = run_expr(switch.cond.deref(), vars, reg);
            ...
            for case in &switch.cases {
                ...
                if matches {
                    ...
                    result = run_expr(&Expression::Block(case.body.clone()), vars, reg);
                    break;
                }
            }
        ...
        Expression::List(list) => {
            ...
            for elem in &list.elems {
                results.push(run_expr(elem, vars, reg));
            }
            Value::List(results)
        }
        Expression::Group(group) => run_expr(group.expr.deref(), vars, reg),
        ...
        Expression::Return(expression) => {
            ...
            let value = run_expr(expression.deref(), vars, reg);
...
fn run_bin_op(binary_op: &BinaryOp, vars: &mut HashMap<String, Value>, reg: &Registry) -> Value {
    let lhs = run_expr(binary_op.left.deref(), vars, reg);
    let rhs = run_expr(binary_op.right.deref(), vars, reg);

    bin_op(binary_op.op.clone(), vars, reg, &lhs, &rhs)
}
...
fn bin_op(
    ...
    match (lhs, rhs, &binary_op) {
        ...
        | (s @ Value::Struct(_), Value::Union(_, u), op @ (Operator::Ne | Operator::Eq)) => {
            match u.clone().deref() {
                inner @ Value::Struct(_) => bin_op(op.clone(), vars, reg, inner, s),
...
fn run_un_op(
    ...
    let expr = run_expr(unary_op.expr.deref(), vars, reg);
